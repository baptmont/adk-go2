...
func (c *RuntimeAPIController) RunSSEHandler(rw http.ResponseWriter, req *http.Request) error {
	...
	if runAgentRequest.StateDelta != nil {
		stateDelta = *runAgentRequest.StateDelta
	}
	resp := r.Run(req.Context(), runAgentRequest.UserId, runAgentRequest.SessionId, &runAgentRequest.NewMessage, *rCfg, runner.WithStateDelta(stateDelta))
